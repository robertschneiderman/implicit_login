...
class SessionForm extends React.Component {
  ...
  componentDidMount() {
    let that = this;
    setInterval(() => {
        that.setState({time: (this.state.time + 1)});
    }, 1000);
  }
  ...
  render() {
    // debugger;
    let style = (Array.isArray(this.props.errors)) ? {visibility: 'hidden'} : {visibility: 'visible'};
    return(
        <div>
            <p className="error" style={style}>{this.props.errors}</p>
